...
export const makeFindPathsTool = (knowledgeGraphIds: KnowledgeGraph["id"][]) =>
  tool({
    description:
      "Find all paths of length 2 between two nodes in the knowledge graph.",
    inputSchema: z.object({
      sourceNodeId: z.string().describe("The ID of the source node."),
      destinationNodeId: z.string().describe("The ID of the destination node."),
    }),
    execute: async ({
      sourceNodeId,
      destinationNodeId,
    }): Promise<FindPathsResponse> => {
      const sourceNodes = await getNodesByIds(knowledgeGraphIds, [
        sourceNodeId,
      ]);
      if (!sourceNodes || sourceNodes.length === 0) {
        throw new Error(
          `Source node with id ${sourceNodeId} not found in the graph.`,
        );
      }
      const destinationNodes = await getNodesByIds(knowledgeGraphIds, [
        destinationNodeId,
      ]);
      if (!destinationNodes || destinationNodes.length === 0) {
        throw new Error(
          `Destination node with id ${destinationNodeId} not found in the graph.`,
        );
      }
      ...
      const paths = await findPathsQuery(
        knowledgeGraphIds,
        sourceNodeId,
        destinationNodeId,
      );

      const toPathNode = (n: Node): PathNode => ({
        id: n.id,
        name: n.name,
        type: n.type,
      });
      ...
      return {
        sourceNode: toPathNode(sourceNode),
        destinationNode: toPathNode(destinationNode),
        pathCount: paths.length,
        paths: paths.map((path) => ({
          legs: path.legs.map((leg) => ({
            tailNode: toPathNode(leg.tailNode),
            edgeType: leg.edgeType,
            headNode: toPathNode(leg.headNode),
          })),
        })),
      };
    },
  ...
  const queryingTools =
    knowledgeGraphIds.length === 0
      ...
      : {
          ...
          findPaths: makeFindPathsTool(knowledgeGraphIds),
        };
  return {
    listAvailableGraphs,
    ...queryingTools,
